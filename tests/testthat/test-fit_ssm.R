...
test_that("fit_ssm catches min.dt error & returns appropriate msg", {
  expect_error(fit_ssm(dkf, min.dt = FALSE, model = "crw", time.step = 72, verbose = 0), 
...
test_that("fit_ssm defaults + crw + KF return foieGras list w 15 elements", {
  f <- fit_ssm(dkf, model = "crw", time.step = 72, optim = "nlminb", verbose = 0)
...
test_that("fit_ssm defaults + crw + LS return foieGras list w 15 elements", {
  skip_on_cran()
  f <- fit_ssm(dls, model = "crw", time.step = 72, verbose = 0)
  expect_s3_class(f$ssm[[1]], "ssm")
  expect_equal(length(f$ssm[[1]]), 15)
  expect_s3_class(f, "fG_ssm")
})
...
test_that("fit_ssm defaults + rw return foieGras list w 15 elements", {
  skip_on_cran()
  f <- fit_ssm(dkf, model = "rw", time.step = 72, map = list(psi = as.factor(NA)), verbose = 0)
  expect_s3_class(f$ssm[[1]], "ssm")
  expect_equal(length(f$ssm[[1]]), 15)
  expect_s3_class(f, "fG_ssm")
})
...
test_that("fit_ssm defaults + rw return foieGras list w 15 elements", {
  skip_on_cran()
  f <- fit_ssm(dls, model = "rw", time.step = 72, map = list(psi = as.factor(NA)), verbose = 0)
  expect_s3_class(f$ssm[[1]], "ssm")
  expect_equal(length(f$ssm[[1]]), 15)
  expect_s3_class(f, "fG_ssm")
})
...
test_that("fit_ssm vmax + crw return foieGras list w 15 elements", {
  skip_on_cran()
  f <- fit_ssm(dkf, vmax=5, model = "crw", time.step = 72, optim = "nlminb", verbose = 0)
  expect_s3_class(f$ssm[[1]], "ssm")
  expect_equal(length(f$ssm[[1]]), 15)
  expect_s3_class(f, "fG_ssm")
})
...
test_that("fit_ssm vmax,ang + crw return foieGras list w 15 elements", {
  skip_on_cran()
  f <- fit_ssm(dkf, vmax=5, ang=c(15,25), model = "crw", time.step = 72, optim = "nlminb", verbose = 0)
  expect_s3_class(f$ssm[[1]], "ssm")
  expect_equal(length(f$ssm[[1]]), 15)
  expect_s3_class(f, "fG_ssm")
})
...
test_that("fit_ssm vmax,ang,min.dt + crw return foieGras list w 15 elements", {
  skip_on_cran()
  f <- fit_ssm(dkf, vmax=5, ang=c(15,25), min.dt=120, model = "crw", time.step = 72, optim = "nlminb", verbose = 0)
  expect_s3_class(f$ssm[[1]], "ssm")
  expect_equal(length(f$ssm[[1]]), 15)
  expect_s3_class(f, "fG_ssm")
})
...
test_that("fit_ssm no spd filter + crw return foieGras list w 15 elements", {
  skip_on_cran()
  f <- expect_warning(fit_ssm(dkf, spdf=FALSE, model = "crw", time.step = 72, optim = "nlminb", verbose = 0),
...
test_that("fit_ssm optim + crw return foieGras list w 15 elements", {
  skip_on_cran()
  f <- fit_ssm(dkf, optim="nlminb", model = "crw", time.step = 72, verbose = 1)
  expect_s3_class(f$ssm[[1]], "ssm")
  expect_equal(length(f$ssm[[1]]), 15)
  expect_s3_class(f, "fG_ssm")
})
...
test_that("fit_ssm full data + crw return foieGras list w 15 elements", {
  skip_on_cran()
  f <- fit_ssm(dkf, verbose=0, fit.to.subset=FALSE, model = "rw", time.step = 72)
  expect_s3_class(f$ssm[[1]], "ssm")
  expect_equal(length(f$ssm[[1]]), 15)
  expect_s3_class(f, "fG_ssm")
})
...
test_that("fit_ssm verbose + crw return foieGras list w 15 elements", {
  skip_on_cran()
  f <- fit_ssm(dkf, verbose=2, model = "crw", time.step = 72, optim = "nlminb")
  expect_s3_class(f$ssm[[1]], "ssm")
  expect_equal(length(f$ssm[[1]]), 15)
  expect_s3_class(f, "fG_ssm")
})
...
test_that("fit_ssm inner.control + crw return foieGras list w 15 elements", {
  skip_on_cran()
  f <- fit_ssm(dkf, inner.control=list(maxit=200), model = "crw", time.step = 72, verbose = 0, optim = "nlminb")
  expect_s3_class(f$ssm[[1]], "ssm")
  expect_equal(length(f$ssm[[1]]), 15)
  expect_s3_class(f, "fG_ssm")
})
...
test_that("fit_ssm multi-track returns foieGras lists w 15 elements", {
  skip_on_cran()
  data(ellies)
  f <- fit_ssm(ellies, vmax=4, model="crw", time.step=72, verbose = 0)
