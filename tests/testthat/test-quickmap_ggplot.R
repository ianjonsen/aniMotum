...
p <- quickmap(fit$ssm[[1]], what = "predicted", obs = TRUE, outlier = TRUE)
test_that("quickmap w all obs plotted returns a ggplot object", {
  expect_s3_class(p, "ggplot")
})

p <- quickmap(fit$ssm[[1]], what = "predicted", obs = TRUE, outlier = TRUE, ext.rng=c(0.1, 0.1))
test_that("quickmap with plot range extended by 10% in x,y returns a ggplot object", {
  expect_s3_class(p, "ggplot")
})

p <- quickmap(fit$ssm[[1]], what = "predicted", obs = TRUE, outlier = TRUE, ext.rng=c(0.1, 0.1), size = 2.5)
test_that("quickmap with re-sized plot symbols returns a ggplot object", {
  expect_s3_class(p, "ggplot")
})

p <- quickmap(fit$ssm[[1]], what = "predicted", obs = TRUE, outlier = TRUE, ext.rng=c(0.1, 0.1), size = 2.5, crs = "+init=epsg:3031 +lon_0=85")
