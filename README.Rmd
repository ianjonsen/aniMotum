...
[![CRAN_Downloads](http://cranlogs.r-pkg.org/badges/grand-total/foieGras?color=brightgreen)](https://cran.r-project.org/package=foieGras)
...
`foieGras` is intended to be as easy to use as possible. Here's an example showing how to quality-control Argos tracking data using parallel processing (via the `future` and `furrr` packages), and infer a behavioural index along the estimated animal tracks:
```{r example, message=FALSE, fig.width=8, fig.height=5}
library(tidyverse)
library(foieGras)
