...
```
However, this may not install any of `{aniMotum}`'s Suggested packages, which add extra functionality such as path re-routing around land. To ensure all Suggested packages, either from R-Universe or CRAN are also installed:
```{r, eval=FALSE}
install.packages("aniMotum", 
