...
[![Coverage status](https://codecov.io/gh/ianjonsen/aniMotum/branch/master/graph/badge.svg)](https://codecov.io/github/ianjonsen/aniMotum?branch=master)
...
```
However, this will not install any of `{aniMotum}`'s Suggested packages, which add extra functionality such as path re-routing around land. To ensure all Suggested packages, either from R-Universe or CRAN are also installed:
```{r, eval=FALSE}
install.packages("aniMotum", 
...
To avoid doing the above every time you want to re-install `{aniMotum}`, you can add my R-Universe repo to your local list of repositories for package download in your `.Rprofile`. This ensures `install.packages` automatically grabs the latest version of `{aniMotum}`
```{r, eval=FALSE}
...
If you don't want to install compiler tools or have trouble getting them to work then you can manually download a binary version of the `{aniMotum}` package for Windows or Mac from here https://ianjonsen.r-universe.dev/ui#package:aniMotum. In R, use the following command to install from the file you've just downloaded (where `path_to_file` is wherever you saved the download:
...
### From GitHub (source)  
To get the very latest but likely unstable `{aniMotum}` version, you can install from GitHub. 
...
## Usage
`{aniMotum}` is intended to be as easy to use as possible. Here's a simple example showing how to fit a move persistence model to Argos tracking data and visualise the result:
