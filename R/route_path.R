...
##'  
##' @param x either a \code{fG_ssm} model fit object or
##' a \code{fG_simfit} object containing simulated paths
##' @param what if using a \code{fG_ssm} object re-route the fitted (typically irregular in time) 
##' or predicted (typically regular in time) locations 
##' 
##' @details
##' If the input is a \code{fG_ssm} object then `route_path` returns a `tibble` 
##' with the same dimensions as either the \code{fG_ssm} fitted or predicted locations.
##' If the input is a \code{fG_simfit} object then `route_path` returns the same nested object
