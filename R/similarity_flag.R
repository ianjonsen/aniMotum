...
similarity_flag <- function(track, sim_track, flag = 2){
  ...
  dist_track <- sp::spDistsN1(pts = cbind(first(track$lon), first(track$lat)),
                              pt = cbind(last(track$lon), last(track$lat)),
                              longlat = T)
  
  dist_sim <- sp::spDistsN1(pts = cbind(first(sim_track$lon), first(sim_track$lat)),
                            pt = cbind(last(sim_track$lon), last(sim_track$lat)),
                            longlat = T)
  
  bear_track <- geosphere::bearing(p1 = cbind(first(track$lon), first(track$lat)),
                                   p2 = cbind(last(track$lon), last(track$lat)))
  
  bear_sim <- geosphere::bearing(p1 = cbind(first(sim_track$lon), first(sim_track$lat)),
