...
plot.foieGras <- function(x, what = c("fitted","predicted"), outlier = FALSE, ...)
{
 ...
 if(inherits(x, "foieGras")) {
    ...
    p1 <- ggplot() +
      ...
      theme_bw() +
      ggtitle(
        label = paste0("id: ", f_sf$id[1], "   model: ", x$pm, "  time.step: ", ts, if (is.numeric(ts))
          " h"),
        subtitle = "fitted states"
      ) +
    ...
    p3 <- ggplot() +
      ...
      geom_point(
        data = switch(what, fitted = f_df, predicted = p_df),
        aes_string("date", "lon"),
        size = 0.75,
        shape = 20,
    ...
    stop("x must be either an `fG` compound tibble or an individual `foieGras` fit object\n")
  }
}
