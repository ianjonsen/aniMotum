...
##'
##' @param x a \code{foieGras} fit object
...
{
  pm <- x$pm
  timeStep <- ifelse(length(x$ts) > 1, "user-supplied prediction times", x$ts)
  nbrStates <- nrow(x$predicted)
  nbStates <- nrow(x$fitted)
  parm <- x$par

  cat("Process model:", pm, "\n")
  ts_txt <- ifelse(length(x$ts) > 1,
                   paste("Time interval:", timeStep, "\n"),
                   paste("Time interval:", timeStep, "hours \n")
                   )
  cat(ts_txt)
  ...
  cat("-------------------\n")
  cat("negative log-likelihood:", ifelse(x$optimiser == "optim", x$opt$value, x$opt$objective), "\n")
