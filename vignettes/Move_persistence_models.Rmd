...
```{r ts1, warning=FALSE, message=FALSE, fig.width=7, fig.height=3}
plot(fmp)
```
```{r map1, fig.width=7, fig.height=6}
...
```{r ts2, fig.width=7, fig.height=3}
fmp <- grab(fit, what = "predicted", normalise = TRUE)
ggplot(fmp, aes(date, g, colour = g)) + 
  geom_line(size = 0.3) + 
  geom_point() +
...
```{r map2, fig.width=7, fig.height=6}
