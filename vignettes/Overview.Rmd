...
`foieGras` expects input data to be provided in one of several possible formats. Here, we outline the default formats but users are not restricted to using these. Input data with variable names and/or column ordering that differs from the defaults can be used, provided the essential minimum data elements are present to allow the SSM's to be fitted: a unique animal/track identifier, date and time of each observation, location quality class, and longitude and latitude (or projected x and y) coordinates. We expand on these and additional variables in the following sections. 

The pre-processing function `format_data()` can be used to correctly structure the input data. This function checks for the presence of the essential variables, converts date-time to `POSIX` format, maps custom variable names onto the default names, and puts the variables into the default column order expected by `foieGras`. Users may choose to call `format_data()` themselves, prior to fitting an SSM with `fit_ssm()`, or they can allow `fit_ssm()` to format the data automatically. We illustrate these approaches in the "Fitting a model" section, below.


#### Argos Least-Squares (LS) location data
We'll start out with the original Argos data, CLS Argos' Least-Squares-based (LS) locations. These data contain the minimum information required by `foieGras`'s SSM's. A minimal input `data.frame` or `tibble`, containing only the information required to fit the SSM, looks like this:
...
- `id` a unique animal (or track) identifier  
- `date` a date-time variable in the form `YYYY-mm-dd HH:MM:SS`. These can be text strings, in which case `foieGras` converts them to `POSIX` format and assumes the timezone is `UTC`. If the date-times are from a local timezone then you must specify this via the `tz` argument to `format_data()` or `fit_ssm()`. If you are unsure about the proper timezone name(s), you can obtain a list of valid timezone names via `OlsonNames()`.
...
Since 2011, the default Argos location data uses CLS Argos' Kalman Filter (KF) algorithm. These data include error ellipse information for each observed location in the form of 3 variables: ellipse semi-major axis length, ellipse semi-minor axis length, and ellipse orientation. A minimal input `data.frame` or `tibble`, containing only the information required to fit the SSM, looks like this:
...
Here, the error ellipse parameters for each observation define the measurement error variances using the the SSM's (Jonsen et al. 2020). Missing error ellipse values are allowed, in which case, those observations are treated as Argos Least-Squares data. 


#### GPS location data
The `foieGras` SSM's can be fit to GPS location data, for example to deal with occasional extreme locations and to time-regularise locations. The input data format is the same as for Argos LS data, but the `lc` class is set to `G` for all GPS locations:
...
The `foieGras` SSM's can be fit to processed light-level geolocation data, when they include longitude and latitude error SD's (`lonerr`, `laterr`; units in degrees). In this case, the `lc` class is set to `GL` for all geolocation observations:
...
We caution users, that light-level geolocation errors can be non-Gaussian, depending on the species, tag technology, and algorithm used to estimate location from the light-level data. For example, we have observed strongly non-Gaussian location uncertainty in tracks of highly migratory tunas that traverse very different water masses in both the vertical and horizontal directions. Because `foieGras`'s SSM's assume bi-variate Normal location errors, fitting to this sort geolocation data would not be appropriate. Geolocation data collected from birds may be less prone to this issue, but we urge users to explore both the uncertainty in their tracking data and the algorithm used to estimate locations from light-level data.


#### Projected location data
Users can provide projected location data as an `sf-tibble` or `sf data.frame`. By default, `foieGras`'s SSM's fit to locations transformed to the Mercator projection and estimated locations are back-transformed to spherical coordinates (long, lat). When an `sf` projected data set is supplied, the `foieGras` SSM's are fit to locations on the supplied projection (`foieGras` ensures that distance units are set to km). Here is an example `sf-tibble` Argos data set:
...
These mixtures are possible because the `foieGras` SSM's apply the most appropriate measurement error model for each observation, based it's supplied location class and error ellipse information (i.e., numeric values vs NA's).
...
These are the minimum arguments required: the input data, the model (`rw`, `crw`, or `mp`) and the time.step (in h) to which locations are predicted. Additional control can be exerted over the data formatting and prefiltering steps, via the `id`, `date`, `lc`, `coord`, `epar` and `sderr` variable name arguments, and via the `vmax`, `ang`, `distlim`, `spdf` and `min.dt` prefiltering arguments. see `?fit_ssm` for details, the defaults for these arguments are quite conservative (for non-flying species), usually leading to relative few observations being flagged to be ignored by the SSM. Additional control over the optimization can also be exerted via the `control = ssm_control()` argument, see `?ssm_control` for details.

#### Custom variable names in data
Users do not need to adhere to the default `foieGras` data formatting presented above, so long as their input data contain the essential variables. The variables can have arbitrary names and be in any order. Here is an example using a custom formatted `data.frame` as input:

```{r fit_ssm2, echo=FALSE, message=FALSE}
sese2_n
```

Note the columns are in a different order the the default `foieGras` order, and the first three variables have different names: `longitude`, `latitude` and `time`. There are two approaches to format these data. The first, is to call `format_data` as a data pre-processing step prior to fitting an SSM. The data are linked to the expected names via the following arguments to `format_data`: `id`, `date`, `lc`, `coord`, `epar` and `sderr`. In this example, we only need to use the `date` and `coord` arguments:
...
Alternatively, we can dispense with the data pre-processing step and rely on `fit_ssm`
to format the data by adding the variable name arguments to the call:
...
Original variable names are not preserved in the output object `fit` but rather transformed to the default expected names. The `grab` function can be used to access data and SSM estimates (see Access Results, below).
