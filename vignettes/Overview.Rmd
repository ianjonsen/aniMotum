...
`foieGras` expects input data to be provided in one of several possible formats. Here, we outline the default formats but users are not restricted to using the default variable names or column ordering. Custom variable names can be specified via variable name arguments in the `fit_ssm` function (see "Custom variable names", below). 

#### Argos Least-Squares (LS) location data
We'll start out with the original Argos data, CLS Argos' Least-Squares-based (LS) locations. These data contain the minimum information required by `foieGras`'s SSM's: (i) a unique track identifier; (ii) the date & time of each observation; (iii) the Argos location quality class of each observation; (iv) the longitude and latitude of each observation. A minimal input `data.frame` or `tibble`, containing only the information required to fit the SSM, looks like this:
...
- `date` a date-time variable in the form `YYYY-mm-dd HH:MM:SS`. These can be text strings, in which case `foieGras` converts date-times to `POSIXt` format and assumes the timezone is `UTC`. If the date-times are from a local timezone then ensure this is encoded in the input data by using, for example, appropriate functions from the `lubridate` R package or `as.POSIXct()`.   
...
require(dplyr)
ellie2 <- ellie %>%
  rename(animal = id, time = date, longitude = lon, latitude = lat,
         location_class = lc,
         semi_major_axis = smaj, 
         semi_minor_axis = smin,
         ellipse_orientation = eor) %>%
  select(latitude, longitude, animal, time, location_class, semi_major_axis, semi_minor_axis, ellipse_orientation)
```

```{r fit_ssm3, eval=TRUE, message=FALSE}
ellie2
```

The data are linked to the expected names via the following arguments to `fit_ssm`: `id`, `date`, `lc`, `coord`, `epar` and `sderr`. In this example, we ignore the `sderr` argument because this information is not contained in the input data:
...
fit <- fit_ssm(ellie2,
               ...
               epar = c("semi_major_axis", "semi_minor_axis", "ellipse_orientation"),
               model = "crw",
               time.step = 24)
```
