...
fit.rw <- fit_ssm(d, spdf = FALSE, model = "rw", time.step = 12, 
                  control = ssm_control(verbose = 0))

# fit SSM `crw` model
fit.crw <- fit_ssm(d, spdf = FALSE, model = "crw", time.step = 12, 
                   control = ssm_control(verbose = 0))
...
plot(I(y+y.err) ~ date, sim.rw, pch=19, col="grey60") # y-values observed with Argos error
points(y ~ date, sim.rw, pch=19, col="dodgerblue") # true y-values
points(y ~ date, loc.rw, pch=19, cex=0.7, col="firebrick") # RW SSM fitted y
points(y ~ date, loc.crw, pch=19, cex=0.4, col="orange") # CRW SSM fitted y
```
...
```{r sim_filter 1, warning=FALSE, message=FALSE, fig.width=7, fig.height=6}
# simulate 50 tracks
...
```{r sim_filter 2, warning=FALSE, message=FALSE, fig.width=7, fig.height=6}
...
## `route_path()` 
We can use `route_path()` to re-route the few simulated track segments that cross land. 
```{r route_path, warning=FALSE, message=FALSE, fig.width=7, fig.height=6}
...
```
In more severe cases, tracks could be first simulated using potential functions, filtered, and then any remaining segments re-routed from land.
