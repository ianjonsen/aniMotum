...
```
these are the minimum arguments required: the input data, the model (`rw` or `crw`) and the time.step (in h) to which locations are predicted (the argument `control = ssm_control(verbose = 0)` turns off parameter optimisation tracing and is included for vignette tidiness). Additional control can be exerted over the prefiltering step, via the `vmax`, `ang`, `distlim`, `spdf` and `min.dt` arguments. see `?fit_ssm` for details, the defaults for these arguments are quite conservative (for non-flying species), usually leading to relative few observations being flagged to be ignored by the SSM. Additional control over the SSM estimation can also be exerted via the `control = ssm_control()` argument, see `?ssm_control` for details.
...
```{r fit plot, fig.width=7.5, fig.height=6}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width=7.5, fig.height=5}
require(patchwork)
...
Here, the time-series residual plots indicate trends in the prediction residuals for both the x and y coordinates, implying a bias in the `rw` SSM fit during the first half of the track. The residuals are also positively autocorrelated, further implying a poor fit of the `rw` SSM to the data. 
