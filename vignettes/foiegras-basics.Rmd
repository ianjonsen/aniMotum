...
```{r ggplot map, fig.width=7, fig.height=7}
require(ggplot2)
require(rnaturalearth)
require(rnaturalearthdata)
## if not installed: install.packages(c("rnaturalearth","rnaturalearthdata"))
## get SSM-predicted locations
ploc_sf <- fit$ssm[[1]]$predicted
prj <- sf::st_crs(ploc_sf)

## get coastline data
coast <- ne_countries(scale=50, returnclass = "sf") 

bounds <- sf::st_bbox(ploc_sf)
## add a buffer to the bounds so plot extends beyond data limits
bounds[c("xmin","xmax")] <- extendrange(bounds[c("xmin","xmax")], f = 0.1)
bounds[c("ymin","ymax")] <- extendrange(bounds[c("ymin","ymax")], f = 0.1)
...
ggplot(data = ploc_sf) +
  ...
  geom_sf(size = 1.25, colour = "dodgerblue")
...
coast <- coast %>% sf::st_transform(., crs = "+init=epsg:3031 +lon_0=85")

## colour locations by date
lab_dates <- with(ploc_sf, seq(min(date), max(date), l = 5)) %>% as.Date()

ggplot() +
  ggspatial::annotation_spatial(data = coast, fill = "grey", lwd = 0) +
  ggspatial::layer_spatial(data = ploc_sf, aes(colour = as.numeric(as.Date(date))), size = 1.25) + 
...
The `sf` class output is easily reprojected but to plot all individuals on a single map, it is useful to cast
the geometries from single POINTS to MULTIPOINTS
