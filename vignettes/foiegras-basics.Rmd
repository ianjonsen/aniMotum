...
```
The predicted values are the state estimates predicted at regular time intervals, specified by `time.step` (here every 24 h). Fitted values (not shown) are the state estimates corresponding to the time of each observation; their time-series are plotted by default - `plot(fit)`.
...
ploc_sf <- grab(fit, what = "predicted") %>% st_transform(., crs = "+init=epsg:3395")
