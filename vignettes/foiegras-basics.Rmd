...
Estimated tracks can be mapped using the `foieGras`-applied projection (here EPSG 3395). We use the `foieGras::grab()` function to access the SSM-predicted values. The land is added using the `rnaturalearth` aand `rnaturalearthdata` packages. The `ggspatial` package's `annotation_spatial` and `layer_spatial` functions ease plotting of `sf` class data.
